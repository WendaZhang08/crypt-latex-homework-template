...
\begin{homeworkProblem}
    ...
    Based on the given Hill cipher encryption function (\textit{E1}) in the question:
    ...
    We discover that the Hill cipher encryption uses the plaintext matrix to multiply the key matrix and then do the modulo calculation. Thus, it could be present as: \\
    ...
    Furthermore, for the given Vernam cipher encryption function (\textit{E2}) in the question:
    ...
    This means that the encryption process is the Hill cipher encryption (\textit{E1}) will be conducted first, then use the acquired ciphertext from \textit{E1} as the plaintext for the Vernam cipher encryption (\textit{E2}) to conduct encryption. Hence, the complete encryption process can be present as: \\
    ...
    # Convert string to numeric matrix for plaintext and ciphertext
    ...
    plainMatrix_inverse = mod_plain_matrix.inverse()
    key_matrix = (plainMatrix_inverse * mod_cipher_matrix).T
    
    print("Complete key matrix:")
    print(key_matrix)
    print("-----------------------------------------------------------")
